...
GAS → Next.js 마이그레이션의 단계별 실행 이력입니다. `CLAUDE.md`가 길어져 분리했습니다.

문서 역할 분담:

- **이 문서**: 무엇을 어떤 순서로 했고 각 단계에서 무엇을 확인했는지 (실행 이력)
- `docs/context-notes.md`: 왜 그렇게 결정했는지 (재사용 가능한 판단 근거)
- `CLAUDE.md`: 지금 코드가 어떤 모습인지 (현재 상태 — 폴더 구조·API·타입·시트 스키마)
...
- [x] 부가 기능: 출석 현황 기간 조회 (`/history`) — 2026-08. 1주 선택 UI를 그대로 둔 채 여러 주 조회 추가
  - [x] `src/app/api/attendance/range/route.ts` — `GET ?from=&to=` → `{ dates: { 날짜: [id...] } }`. Attendance **1회 읽기**로 범위 전체 반환(주별 N회 호출 금지). 세션 필터 없음 — 화면이 roster로 좁힘
  - [x] `src/lib/attendance-range.ts` — `resolveRange`(기준일+기간 → 구간, 올해 목록 밖으로 안 나가게 인덱스 clamp) / `buildRangeStats`(id→출석횟수, 예배일 목록, 주차별 합계)
  - [x] `src/hooks/useAttendanceRange.ts` — 읽기 전용. **종료일이 가장 최근 예배일일 때만** 30초 폴링(지나간 구간은 값이 안 변함). `useAttendance`(토글 훅)는 무변경 — `enabled`로만 스위치
  - [x] `src/components/history/ServiceDateSelector.tsx` — 기준일 + 기간 길이. `직접 지정`에서 시작/종료 역전 시 `AlertDialog`로 경고 후 선택 자동 복귀
  - [x] `GroupAttendanceChart`·`AttendanceListModal` props 교체 (`attendedIds: Set` → `attendCounts: Map` + `weeks`) — **weeks=1이면 계산이 기존과 동일**해 1주 모드에 회귀 경로 없음
  - [x] `WeeklyTrendChart`(면적+평균선, 클릭 시 그 주로 이동) / `AttendanceHighlights`(개근·결석 칩) / `SummaryBar`의 `caption` prop 추가 — 플로팅 바에도 표시
...
- [x] 코드 리뷰 후속 (`/history` 기간 조회) — 2026-08. 기능 전체를 리뷰해 지적 3건 처리
  - [x] 지적 ①② (비일요일 기준일로 인한 화살표 잠김 / `to` 바꿔치기) — **고치지 않음**으로 종결.
    앱이 비일요일을 쓸 수 있는 경로가 없고(`/`는 `mostRecentSunday()`, `/members`·`/history`는 `sundaysThisYear()`),
    실데이터도 2026년 출석 32일 전부 일요일(비일요일 0일). 대신 `docs/context-notes.md`에
    **운영 방침과 "방어하지 않기로 한 결정"** 을 기록해 같은 지점을 다시 파고들지 않도록 함
  - [x] 배지의 조회 종료일을 `date`(원본 상태) → `to`(`resolveRange` 결과)로 교체 — 같은 값을 두 출처가
    읽던 구조를 하나로 합침. 배지 문구 vs 실제 API 요청 파라미터 대조로 검증
    (프리셋 `2026-07-19~2026-08-09`, 맞춤 `2026-05-03~2026-08-09` 모두 일치)
  - [x] 지적 ③ 수정 — 기간 전환 중 `keepPreviousData` 때문에 `13주 중 4주 예배` 같은 거짓 문구가
    약 550ms 노출되던 문제. `serviceWeeks` 조건에 `!range.isPlaceholderData` 추가로 로딩 중 꼬리말만 감춤
    - 검증: 40ms 간격 50회 샘플링 → 거짓 문구 **0회**. 폴링 켜진 상태 38초간 76회 샘플링 → 꼬리말 사라짐 **0회**
    - `isFetching`은 30초 폴링에도 반응해 문구가 깜빡이므로 쓰지 않음
  - [x] `npm run build` 통과
  - 결정 근거는 `docs/context-notes.md`의 "출석 날짜는 항상 일요일이다", "코드 리뷰 후속" 항목 참조
